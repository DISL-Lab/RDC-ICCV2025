...
def main(args, logger, repeat=1):
    ...
    for i in range(repeat):
        ...
        best_acc, acc = train(args, model, train_loader, val_loader, i, plotter, logger)
...
def train(args, model, train_loader, val_loader, repeat_idx=0, plotter=None, logger=None):
    ...
    for epoch in range(cur_epoch + 1, args.epochs + 1):
        ...
        if epoch % args.epoch_print_freq == 0:
            ...
            if is_best:
                ...
                best_acc5 = acc5
                if logger != None:
                    logger(f'Best accuracy (top-1 and 5): {best_acc1:.2f} {best_acc5:.2f}')
        ...
        if args.save_ckpt and (is_best or (epoch == args.epochs)):
            ...
            save_checkpoint(args.save_dir, state, is_best, repeat_idx)
        scheduler.step()
...
def train_epoch(args,
    ...
    if (epoch % args.epoch_print_freq == 0) and (logger is not None):
        logger(
            '(Train) [Epoch {0}/{1}] {2} Top1 {top1.avg:.2f}  Top5 {top5.avg:.2f}  Loss {loss.avg:.3f}'
            .format(epoch, args.epochs, get_time(), top1=top1, top5=top5, loss=losses))

    return top1.avg, top5.avg, losses.avg

def train_epoch_semisupervised(args,
                train_loader,
                noisy_loader,
                model,
                optimizer,
    ...
    for i, (img_real_, lab_real_) in enumerate(train_loader):
        data_time.update(time.time() - end)
        
        if train_loader.device == 'cpu':
            img_real_ = img_real_.cuda()
            lab_real_ = lab_real_.cuda()

        try:
            img_noisy_, _ = next(iter(noisy_loader))
        except StopIteration:
            noisy_iter = iter(noisy_loader)  # 다시 초기화
            img_noisy_, _ = next(noisy_iter)
        img_noisy_ = img_noisy_.cuda()

        with torch.no_grad():
            img_noisy_augmented = aug(img_noisy_)

            pred_logits = model(img_noisy_)
            pseudo_labels = sharpen(pred_logits, 0.5)

            pred_logits_aug = model(img_noisy_augmented)
            pseudo_labels_aug = sharpen(pred_logits_aug, 0.5)

            output_real_ = model(img_real_)

        mixed_images, mixed_labels = mixup(
            img_real_, img_noisy_,
            F.one_hot(lab_real_, num_classes=args.nclass).float(),
            pseudo_labels
        )
        ...
        # Regularization
        prior = torch.ones(args.nclass) / args.nclass
        prior = prior.cuda()
        ...
        # Compute loss
        loss = 0
        loss += -torch.mean(torch.sum(F.log_softmax(output_real_, dim=1) * 
                                    F.one_hot(lab_real_, num_classes=args.nclass).float(), dim=1))
        ...
        # measure accuracy and record loss
        acc1, acc5 = accuracy(output_real_.data, lab_real_, topk=(1, 5))

        losses.update(loss.item(), output_real_.size(0))
        top1.update(acc1.item(), output_real_.size(0))
        top5.update(acc5.item(), output_real_.size(0))
...
def validate(args, val_loader, model, criterion, epoch, logger=None):
    ...
    if logger is not None:
        logger(
            '(Test ) [Epoch {0}/{1}] {2} Top1 {top1.avg:.2f}  Top5 {top5.avg:.2f}  Loss {loss.avg:.3f}'
            .format(epoch, args.epochs, get_time(), top1=top1, top5=top5, loss=losses))
    return top1.avg, top5.avg, losses.avg
...
def save_checkpoint(save_dir, state, is_best, repeat_idx):
    os.makedirs(save_dir, exist_ok=True)
    if is_best:
        ckpt_path = os.path.join(save_dir, 'model_best_'+str(repeat_idx)+'.pth.tar')
    else:
        ckpt_path = os.path.join(save_dir, 'checkpoint_'+str(repeat_idx)+'.pth.tar')
