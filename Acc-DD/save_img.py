...
class Synthesizer():
    ...
    def test(self, args, val_loader, logger, bench=True):
        ...
        test_data(args, loader, val_loader, repeat=3, test_resnet=False, logger=logger)
...
def diffaug(args, device='cuda'):
    ...
    elif args.dataset[:10]=='cifar100n_':
        normalize = utils.Normalize(mean=MEANS['cifar100'], std=STDS['cifar100'], device=device)
    else:
...
def matchloss(args, img_real, img_syn, lab_real, lab_syn, model):
    ...
    if args.match == 'feat':
        ...
        print(img_real.size())
        flops, params = profile(model, (img_real, ))
        print("img_syn: %.2f M" % (flops / 1000000.0))
...
def save_img(save_dir, img, unnormalize=True, max_num=200, size=(64,64), nrow=10, dataname='imagenet'):
    print(img.size())
    img = img[30].detach()
    if unnormalize:
...
def condense(args, logger, device='cuda'):
    ...
    save_img(os.path.join('vis_cifar10', 'init_4.png'),
             synset.data,
             unnormalize=False,
...
if __name__ == '__main__':
    ...
    # data = torch.load('results/cifar10/conv3in_grad_mse_inloop100_cut_niter2000_factor2_lr0.02_mix_ipc10_pretrainweight-perturbation-large-2_mnum5/data_best.pt')
    # print(len(data))