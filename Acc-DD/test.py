    ...
    if 'idc' in args.slct_type:
        ...
        if name == '':
            if args.dataset[:5] == 'cifar':
                name = f'{args.dataset}/resnet18in_grad_mse_nd2000_cut_niter2000_factor{args.factor}_lr0.005_{init}'
        ...
        path_list = [f'{args.dataset}/dsa/res_DSA_{args.dataset}_ResNet_{args.ipc}ipc']
    elif args.slct_type == 'kip':
        path_list = [f'{args.dataset}/kip/kip_ipc{args.ipc}']
    else:
        path_list = ['']
...
def load_data_path(args):
    ...
    if args.dataset == 'imagenet':
        ...
        # Load condensed dataset
        if 'idc' in args.slct_type:     # todo
            # todo revise data target
            # data, target = torch.load(os.path.join(f'{args.save_dir}', 'data.pt'))
            save_dir_here = f'results/{args.dataset}/resnet18in_grad_mse_inloop100_cut_niter2000_factor2_lr0.02_mix_ipc10_pretrainweight-perturbation-large-2_mnum5/data_best.pt'
            data, target = torch.load(save_dir_here)
            print("Load condensed data ", save_dir_here, data.shape)
            ...
            if args.dataset == 'cifar10':
                ...
                train_dataset = torchvision.datasets.CIFAR10(args.data_dir, train=True, transform=train_transform)
...
def test_data(args,
    ...
    for model_fn in model_fn_ls:
        ...
        for _ in range(repeat):
            ...
            torch.save(model.state_dict(), f'{args.dataset}-syn-resnet18.pt')
            best_acc_l.append(best_acc)
            acc_l.append(acc)
        logger(
            f'Repeat {repeat} => Best, last acc, std: {np.mean(best_acc_l):.2f} {np.mean(acc_l):.2f} {np.std(best_acc_l):.2f}\n')
    
    return np.mean(best_acc_l), np.mean(acc_l)          # return best_acc, last_acc
