...
def get_dataset(dataset, data_path, batch_size=1, subset="imagenette", args=None):
    ...
    if dataset == 'CIFAR10':
        ...
        if args.zca:
            ...
            transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])        
        dst_train = datasets.CIFAR10(data_path, train=True, download=True, transform=transform) # no augmentation
        data = torch.load(os.path.join(data_path, 'CIFAR10N_asym.pt'), map_location='cpu')
        dst_train.targets = data
        dst_test = datasets.CIFAR10(data_path, train=False, download=True, transform=transform)
        class_names = dst_train.classes
        class_map = {x:x for x in range(num_classes)}

    elif dataset == 'CIFAR10N_asym_20':
        channel = 3
        im_size = (32, 32)
        num_classes = 10
        mean = [0.4914, 0.4822, 0.4465]
        ...
        if args.zca:
            ...
            transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])        
        dst_train = datasets.CIFAR10(data_path, train=True, download=True, transform=transform) # no augmentation
        data = torch.load(os.path.join(data_path, 'CIFAR10N_asym_20.pt'), map_location='cpu', weights_only=True)
        dst_train.targets = data
        dst_test = datasets.CIFAR10(data_path, train=False, download=True, transform=transform)
        class_names = dst_train.classes
        class_map = {x:x for x in range(num_classes)}

    elif dataset == 'CIFAR10N_sym_40':
        channel = 3
        im_size = (32, 32)
        num_classes = 10
        mean = [0.4914, 0.4822, 0.4465]
        ...
        if args.zca:
            ...
            transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])        
        dst_train = datasets.CIFAR10(data_path, train=True, download=True, transform=transform) # no augmentation
        data = torch.load(os.path.join(data_path, 'CIFAR10N_sym.pt'), map_location='cpu')
        dst_train.targets = data
        dst_test = datasets.CIFAR10(data_path, train=False, download=True, transform=transform)
        class_names = dst_train.classes
        class_map = {x:x for x in range(num_classes)}

    elif dataset == 'CIFAR10N_sym_20':
        channel = 3
        im_size = (32, 32)
        num_classes = 10
        mean = [0.4914, 0.4822, 0.4465]
        ...
        if args.zca:
            ...
            transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])        
        dst_train = datasets.CIFAR10(data_path, train=True, download=True, transform=transform) # no augmentation
        data = torch.load(os.path.join(data_path, 'CIFAR10N_sym_20.pt'), map_location='cpu', weights_only=True)
        dst_train.targets = data
        dst_test = datasets.CIFAR10(data_path, train=False, download=True, transform=transform)
        class_names = dst_train.classes
        class_map = {x:x for x in range(num_classes)}

    elif dataset == 'CIFAR10N_ran1':
        channel = 3
        im_size = (32, 32)
        num_classes = 10
        mean = [0.4914, 0.4822, 0.4465]
        ...
        if args.zca:
            ...
            transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
        dst_train = datasets.CIFAR10(data_path, train=True, download=True, transform=transform) # no augmentation
        data = torch.load(os.path.join(data_path, 'CIFAR-10_human.pt'), map_location='cpu')
        labels = data['random_label1']
        dst_train.targets = labels
        dst_test = datasets.CIFAR10(data_path, train=False, download=True, transform=transform)
        class_names = dst_train.classes

    elif dataset == 'CIFAR10N_worse':
        channel = 3
        im_size = (32, 32)
        num_classes = 10
        mean = [0.4914, 0.4822, 0.4465]
    ...
    elif dataset == 'CIFAR100N_asym_20':
        channel = 3
        im_size = (32, 32)
        num_classes = 100
        mean = [0.5071, 0.4866, 0.4409]
        ...
        transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
        dst_train = datasets.CIFAR100(data_path, train=True, download=True, transform=transform) # no augmentation
        data = torch.load(os.path.join(data_path, 'CIFAR100N_asym_20.pt'), map_location='cpu')
        dst_train.targets = data
        dst_test = datasets.CIFAR100(data_path, train=False, download=True, transform=transform)
        class_names = dst_train.classes

    elif dataset == 'CIFAR100N_asym_40':
        channel = 3
        im_size = (32, 32)
        num_classes = 100
        mean = [0.5071, 0.4866, 0.4409]
        ...
        transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
        dst_train = datasets.CIFAR100(data_path, train=True, download=True, transform=transform) # no augmentation
        data = torch.load(os.path.join(data_path, 'CIFAR100N_asym_40.pt'), map_location='cpu')
        dst_train.targets = data
        dst_test = datasets.CIFAR100(data_path, train=False, download=True, transform=transform)
        class_names = dst_train.classes

    elif dataset == 'CIFAR100N_sym_20':
        channel = 3
        im_size = (32, 32)
        num_classes = 100
        mean = [0.5071, 0.4866, 0.4409]
        ...
        transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
        dst_train = datasets.CIFAR100(data_path, train=True, download=True, transform=transform) # no augmentation
        data = torch.load(os.path.join(data_path, 'CIFAR100N_sym_20.pt'), map_location='cpu')
        dst_train.targets = data
        dst_test = datasets.CIFAR100(data_path, train=False, download=True, transform=transform)
        class_names = dst_train.classes

    elif dataset == 'CIFAR100N_sym_40':
        channel = 3
        im_size = (32, 32)
        num_classes = 100
        mean = [0.5071, 0.4866, 0.4409]
        ...
        transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
        dst_train = datasets.CIFAR100(data_path, train=True, download=True, transform=transform) # no augmentation
        data = torch.load(os.path.join(data_path, 'CIFAR100N_sym_40.pt'), map_location='cpu')
        dst_train.targets = data
        dst_test = datasets.CIFAR100(data_path, train=False, download=True, transform=transform)
        class_names = dst_train.classes

    elif dataset == 'CIFAR100N_noisy':
        channel = 3
        im_size = (32, 32)
        num_classes = 100
        mean = [0.5071, 0.4866, 0.4409]
        ...
        transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=mean, std=std)])
        dst_train = datasets.CIFAR100(data_path, train=True, download=True, transform=transform) # no augmentation
        data = torch.load(os.path.join(data_path, 'CIFAR-100_human.pt'), map_location='cpu')
        labels = data['noisy_label']
        dst_train.targets = labels
        dst_test = datasets.CIFAR100(data_path, train=False, download=True, transform=transform)
        class_names = dst_train.classes
