...
def main():
    ...
    parser.add_argument('--outer_loop', type=int, default=1, help='Outer loop steps')
    parser.add_argument('--inner_loop', type=int, default=1, help='Inner loop steps')
    ...
    parser.add_argument('--batch_train', type=int, default=256, help='Batch size for network training')
    parser.add_argument('--last', type=int, default=100, help='Auxiliary parameter')
    parser_bool(parser, 'net_decay', True)
    parser.add_argument('--model_train_steps', type=int, default=10, help='Student training steps')
    parser.add_argument('--net_num', type=int, default=100, help='Max student nets')
    parser.add_argument('--net_begin', type=int, default=0, help='Student pool index start')
    parser.add_argument('--net_end', type=int, default=100000, help='Student pool index end')
    parser.add_argument('--ij_selection', type=str, default='random', help='Net selection policy')
    parser.add_argument('--net_generate_interval', type=int, default=30, help='Pool refresh interval')
    parser.add_argument('--fetch_net_num', type=int, default=2, help='Fetched nets per cycle')
    parser.add_argument('--embed_last', type=int, default=-1, help='Embedding layer index')
    parser_bool(parser, 'syn_ce', True)
    parser.add_argument('--ce_weight', type=float, default=0.1, help='Synthetic CE weight')
    parser.add_argument('--optim', type=str, default='sgd', help='Optimizer for image tensors')
    parser.add_argument('--train_net_num', type=int, default=2, help='Nets used for image update')
    parser.add_argument('--aug_num', type=int, default=1, help='Augmentation draws')
    parser.add_argument('--net_push_num', type=int, default=1, help='Nets pushed per refresh')
    ...
    checkpoint_dir = args.save_path + '/IDM_{}_ipc{}_{}/'.format(args.dataset, args.ipc, args.model)
    ...
    logging.info(f'eval_it_pool: {eval_it_pool}')
    ...
    for exp in range(args.num_exp):
        ...
        logging.info("Shape of Image_SYN: {}".format(image_syn.shape))
        ...
        for it in range(args.Iteration+1):
            ...
            if it in eval_it_pool[1:]:
                for model_eval in model_eval_pool:
                    ...
                    logging.info(f'DSA augmentation strategy:\n{args.dsa_strategy}')
                    logging.info(f'DSA augmentation parameters:\n{args.dsa_param.__dict__}')
                    ...
                    if best_acc < np.mean(accs):
                        ...
                        with open(best_synset_filename, 'wb') as pkl_file:
                            ...
                            logging.info("Saving best synset with accuracy: {}".format(np.mean(accs)))
            ...
            if it%10 == 0:
                logging.info('%s iter = %04d, loss = syn:%.4f, net_list size = %s, metrics = syn:%.4f/real:%.4f, syn acc = syn:%.4f' % (get_time(), it, \
