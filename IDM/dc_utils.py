...
from utils_edc import accuracy, AverageMeter, rand_bbox, random_indices
from augment_edc import DiffAug
...
def get_network(model, channel, num_classes, im_size=(32, 32), v2_setting=False):
    ...
    if model == 'MLP':
        ...
        net = ConvNet_6(channel=channel, num_classes=num_classes, net_width=net_width, net_depth=6, net_act=net_act, net_norm=net_norm, net_pooling=net_pooling, im_size=im_size)
    elif model == 'ConvNet_Pooling':
        net = ConvNet_Pooling(channel=channel, num_classes=num_classes, net_width=net_width, net_depth=net_depth, net_act=net_act, net_norm=net_norm, net_pooling=net_pooling, im_size=im_size)
    elif model == 'ConvNet_BSR':
        net = ConvNet_BSR(channel=channel, num_classes=num_classes, net_width=net_width, net_depth=net_depth, net_act=net_act, net_norm=net_norm, net_pooling=net_pooling, im_size=im_size)
        ...
        net = ConvNet_GBN(channel=channel, num_classes=num_classes, net_width=net_width, net_depth=net_depth, net_act=net_act, net_norm=net_norm, net_pooling=net_pooling, im_size=im_size)
    elif model == 'ConvNet_Adv':
        net = ConvNet_Adv(channel=channel, num_classes=num_classes, net_width=net_width, net_depth=net_depth, net_act=net_act, net_norm=net_norm, net_pooling=net_pooling, im_size=im_size)
    elif model == 'ConvNet_fc':
        net = ConvNet_fc(channel=channel, num_classes=num_classes, net_width=net_width, net_depth=net_depth, net_act=net_act, net_norm=net_norm, net_pooling=net_pooling, im_size=im_size)
    elif model == 'LeNet':
...
def evaluate_synset(it_eval, net, images_train, labels_train, testloader, args, upsample=False, pooling=False, partial=None, aug=True, tqdm_bar=False):
    net = net.to(args.device)
    ...
    start = time.time()
    if tqdm_bar: training_range = tqdm(range(Epoch+1), ncols=0)
    else: training_range = range(Epoch + 1)
    for ep in training_range:
        if not upsample:
            loss_train, acc_train = epoch('train', trainloader, net, optimizer, criterion, args, aug = aug, pooling=pooling)
        else:
...
def evaluate_synset_edc(it_eval, net, images_train, labels_train, testloader, args, upsample=False, pooling=False, partial=None, aug=True, tqdm_bar=False):
    net = net.to(args.device)
    images_train = images_train.to(args.device)
    labels_train = labels_train.to(args.device)
    lr = float(args.lr_net)
    ...
    best_acc, acc = train_edc(args, net, trainloader, testloader)
    ...
    # return net, acc_train, acc_test
    return net, best_acc, acc

def train_edc(args, model, train_loader, val_loader, plotter=None, logger=None):
    ...
    scheduler = optim.lr_scheduler.MultiStepLR(
        optimizer, milestones=[2 * 300 // 3, 5 * 300 // 6], gamma=0.2)
    ...
    for epoch in tqdm(range(cur_epoch + 1, 300 + 1), ncols=0):
        acc1_tr, _, loss_tr = train_epoch_edc(args,
                                          train_loader,
                                          model,
                                          criterion,
                                          optimizer,
        ...
        if epoch % 100 == 0:
            acc1, acc5, loss_val = validate_edc(args, val_loader, model, criterion, epoch, logger)
...
def train_epoch_edc(args,
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        r = np.random.rand(1)
        if r < 1.0 and mixup == 'cut':
            # generate mixed sample
            args.beta = 1.0
            lam = np.random.beta(args.beta, args.beta)
            rand_index = random_indices(target, nclass=100)
...
def validate_edc(args, val_loader, model, criterion, epoch, logger=None):
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
...
def evaluate_synset_mixup(it_eval, net, images_train, labels_train, testloader, args, num_classes):
    ...
    for ep in range(Epoch+1):
        loss_train, acc_train = epoch_mixup('train', trainloader, net, optimizer, criterion, args, aug = True, mloader1=mixup_loader1, mloader2=mixup_loader2, num_classes=num_classes)
        if ep in lr_schedule:
            lr *= 0.1
            optimizer = torch.optim.SGD(net.parameters(), lr=lr, momentum=0.9, weight_decay=0.0005)
...
def number_sign_augment(image_syn, label_syn):
    ...
    a, b, c, d = F.interpolate(a, scale_factor=2, mode='bilinear'), F.interpolate(b, scale_factor=2, mode='bilinear'), \
        F.interpolate(c, scale_factor=2, mode='bilinear'), F.interpolate(d, scale_factor=2, mode='bilinear')
