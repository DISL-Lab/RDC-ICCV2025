...
class VGG(nn.Module):
    ...
    def embed_channel_avg(self, x, last=-1):
        """
        Extract embeddings from VGG by averaging the channels up to a specified layer.
        """
        modulelist = list(self.features.children())
        
        # Apply each module sequentially up to the `last` layer
        for idx, module in enumerate(modulelist[:last]):
            x = module(x)
        
        # Average the channels and flatten the output
        out = x.mean(dim=1)  # Average across channels
        out = out.view(out.size(0), -1)  # Flatten
        return out

    def embed_by_layer(self, x, layer_idx=100, last=100):
        """
        Extract embeddings from VGG at a specific layer or up to a specific layer.
        """
        layer_idx = min(layer_idx, last)
        modulelist = list(self.features.children())
        
        # Apply layers up to `last`, stopping at `layer_idx` if specified
        for idx, module in enumerate(modulelist[:last]):
            x = module(x)
            if idx == layer_idx - 1:  # Return after the specified layer
                return x.view(x.size(0), -1)
        
        # If layer_idx is not reached, return the final average pooled output
        out = x.mean(dim=(2, 3))  # Global average pooling (spatial dimensions)
        out = out.view(out.size(0), -1)
        return out
...
class ResNet(nn.Module):
    ...
    def embed_channel_avg(self, x, last=-1):
        ...
        # Apply each layer sequentially up to `last`
        for idx, module in enumerate(modulelist[:last]):
            x = module(x)
        out = x.view(x.size(0), -1)
        return out
